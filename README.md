...
6. When a *Customer* is ready to purchase a product from a *Merchant*, the customer leverages a gnosis safe relay service function to call the `PrepaidCardManager.payMerchant` method. This method will transfer the tokens used to pay a merchant into the `RevenuePool` contract.
