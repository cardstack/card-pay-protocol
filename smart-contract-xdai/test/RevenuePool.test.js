...
contract("Test Revenue Pool contract", (accounts) => {
	...
	it("merchant resigter by tally", async () => {
		await revenuePool.registerMerchant(lw.accounts[0], { from: tally });
	...
	it("merchant resigter not by tally", async () => {
		try {
			await revenuePool.registerMerchant(lw.accounts[0], {
				from: accounts[2],
			});
			walletOfMerchant = await revenuePool.getMerchantWallet(
				lw.accounts[0]
			);
	...
	it("pay 1 DAI CPXD token to pool and mint SPEND token for merchant wallet", async () => {
		...
		let balanceOfMerchantSPEND = await spendToken.balanceOf(
			walletOfMerchant
		);
		...
		assert.equal(
			balanceOfMerchantSPEND.toString(),
			utils.fromDAICPXD2SPEND(1, 100)
		);
	});
	...
	it("redeem 100 SPEND for merchant by tally", async () => {
		...
		await revenuePool.redeemRevenue(
			lw.accounts[0],
			daicpxdToken.address,
			amountSPEND,
			{ from: tally }
		...
		let balanceOfMerchantDAICPXD = await daicpxdToken.balanceOf(
			walletOfMerchant
		);

		assert.equal(
			balanceOfMerchantDAICPXD.toString(),
			web3.utils.toWei("1")
		);
	...
	it("pay 1 DAI CPXD and receive address is not merchant", async () => {
		...
		try {
			await daicpxdToken.transferAndCall(
				revenuePool.address,
				amount,
				web3.eth.abi.encodeParameter("address", lw.accounts[2])
			);
			assert.ok(false);
		} catch (err) {
