...
contract("Test Prepaid Card Manager contract", (accounts) => {
	...
	it("Issuer create prepaid card have amount = 1 token DAI CPXD", async () => {
		...
		assert.equal(
			safeTxHash.toString(),
			executeSuccess[0]['txHash'].toString(),
		...
		let prepaidCard = await getGnosisSafeFromEventLog(safeTx, prepaidCardManager.address);
		...
		await TokenHelper.isEqualBalance(daicpxdToken, walletOfIssuer.address, TokenHelper.amountOf(19).sub(paymentActual));
	...
	it("Issure create card with value less than 1 token", async() => {
		...
		let payloads = daicpxdToken.contract.methods
			.transferAndCall(
				...
				ContractHelper.encodeCreateCardsData(walletOfIssuer.address, [TokenHelper.amountOf(0)])
			).encodeABI();
		...
		let executeFailed = getParamsFromEvent(safeTx, eventABIs.EXECUTION_FAILURE, walletOfIssuer.address);
	...
	it('issuer create number card is zero', async () => {
		...
		let payloads = daicpxdToken.contract.methods
			.transferAndCall(
				...
				ContractHelper.encodeCreateCardsData(walletOfIssuer.address, [])
			).encodeABI();
	...
	it("Issuer sell card with 5 daicpxd (prepaidCards[2]) to customer", async () => {
		...
		let executeSuccess = getParamsFromEvent(safeTx, eventABIs.EXECUTION_SUCCESS, walletOfIssuer.address);
		assert.equal(safeTxHash.toString(), executeSuccess[0]['txHash'].toString());
	...
	it("split card from 2 token to 2 card with 1 token for each", async () => {
		...
		let amounts = [1, 1].map(amount => TokenHelper.amountOf(amount).toString());
		...
		const signature = await signSafeTransaction(
			...
			ZERO_ADDRESS,
			ZERO_ADDRESS,
			await walletOfIssuer.nonce.call(),
			issuer,
			walletOfIssuer
		);

		let txHash = await walletOfIssuer.getTransactionHash(
			multiSend.address,
			0,
			payloads,
			1,
		...
		let tx = await walletOfIssuer.execTransaction(
			multiSend.address,
			0,
			payloads,
			1,
		...
		let executeSuccess = getParamsFromEvent(tx, eventABIs.EXECUTION_SUCCESS, walletOfIssuer.address);
		
		assert.equal(txHash.toString(), executeSuccess[0]['txHash'].toString());
		
		let successPrepaidCards = await getGnosisSafeFromEventLog(tx, prepaidCardManager.address);
		assert.equal(successPrepaidCards.length, 2);
		
		assert.isTrue(await successPrepaidCards[0].isOwner(walletOfIssuer.address));
		assert.isTrue(await successPrepaidCards[1].isOwner(walletOfIssuer.address));
	})
    ...
    it("Payable token add and remove", async() => {
        ...
        let currentPayableTokens = await prepaidCardManager.getTokens();

        assert.deepEqual([mockPayableTokenAddr], currentPayableTokens);
    })
});