...
deployWithVerification() {
  ...
  attempts=0
  maxAttempts=5
  while [ $isVerified == "false" ] && [ $attempts -lt $maxAttempts ]; do
    instanceAddress=$(deploy $network $contractName "${args}")
    attempts=$attempts + 1
    ...
    isVerified=$(verifyImplementation $network $contractName $implementationAddress || echo "false")
    if [ "$isVerified" != "false" ]; then
      break
    elif [ $attempts -lt $maxAttempts ]; then
      echo "unable to verify contract $contractName at $implementationAddress, retrying..."
    fi
  done
  if [ $attempts == $maxAttempts ]; then
    echo "Failed to verify contract $contractName at $implementationAddress after $maxAttempts attempts"
    exit 1
  fi
  echo $instanceAddress
}
...
## $1 = Network
## $2 = Contract ID
getLastDeployedProxyAddress() {
  network=$1
  id=$2
  conf=''
  if [ -f "./.openzeppelin/addresses-${network}.json" ]; then
    conf="$(cat ./.openzeppelin/addresses-${network}.json)"
  fi
  if [ -n "$conf" ]; then
    result=$(echo "$conf" | jq -r ".${id}.proxy")
    if [ "$result" != null ]; then
      echo "$result"
    fi
  fi
...
## $1 = Network
## $2 = Command string: contract_name:contract_id.method arg1,arg2,arg3
##                      contract_name.method arg1,arg2,arg3
execCommand() {
  network=$1
  command="$2"
  action=${command%% *}
