...
let calcDataGasCosts = function (dataString) {
  const reducer = (accumulator, currentValue) =>
    (accumulator += byteGasCosts(currentValue));
...
let estimateBaseGas = function (
  ...
  let payload = safe.contract.methods
    .execTransaction(
      to,
      value,
      data,
    ...
    .encodeABI();
  let baseGasEstimate =
    calcDataGasCosts(payload) + signatureCost + (nonce > 0 ? 5000 : 20000);
...
let executeTransactionWithSigner = async function (
  ...
  let txGasEstimate = 0;
  let estimateData = safe.contract.methods
  ...
  expect(transactionHash).to.equal(event.args.txHash);
  if (txGasEstimate > 0) {
    let maxPayment = (baseGasEstimate + txGasEstimate) * gasPrice;
    console.log(
      "    User paid",
      event.args.payment.toString(),
      "after signing a maximum of",
      maxPayment
    );
    expect(maxPayment >= event.args.payment).to.be.ok(
      "Should not pay more than signed"
    );
  } else {
